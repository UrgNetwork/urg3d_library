...
#define AUXILARY_LOG_FILE_NAME "yvt_x002_auxilary_log.csv"
...
static void clear_yvt_sensor_ax_data_buffer(void)
{
    ...
    for (data_index = 0; data_index < MAXIMUM_CAPTURING_COUNT * MAXIMUM_RECORD_TIMES; ++data_index) {
        ...
        for (data_type = 0; data_type < NUMBER_OF_YVT_X002_AUXILARY_DATA; ++data_type) {
...
int main(int argc, char *argv[])
{
    ...
    urg3d_t urg;
    urg3d_auxilary_data_t auxilary_data;
    ...
    while(1) {
        ...
        if(urg3d_next_receive_ready(&urg)) {
            ...
            if(urg3d_high_get_auxilary_data(&urg, &auxilary_data) > 0) {

                if (is_yvt_x002_valid_auxilary_data(&auxilary_data) > 0) {
                    ++capturing_count;
                    ...
                    capturing_record_count = auxilary_data.record_count;
                    ...
                    for (record_index = 0; record_index < capturing_record_count; ++record_index) {
                        set_value_unit_of_yvt_x002(&auxilary_data.records[record_index],
    ...
    file_descriptor = fopen(AUXILARY_LOG_FILE_NAME, "w");
