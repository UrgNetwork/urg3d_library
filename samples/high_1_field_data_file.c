    ...
    while(1) {
        ...
        if(urg3d_next_receive_ready(&urg)) {
            ...
            if(urg3d_high_get_measurement_data(&urg, &measurement_data) > 0) {
                 ...
                 * wait for first data
                 */
                if(prev_frame == -1 && prev_field == -1) {
                    /*
                     * check line number
                     */
                    if(measurement_data.line_number == 0) {
                        prev_frame = measurement_data.frame_number;
                        prev_field = measurement_data.h_field_number;
                    }
                }
                /*
                 * start field data
                ...
                if(prev_frame != -1 && prev_field != -1) {
                     ...
                     */
                    if(prev_frame != measurement_data.frame_number || prev_field != measurement_data.h_field_number) {
                        break;
                    }
                    /*
                     * add data for buffer
