...
void print_debug_info(const urg3d_t* urg, const urg3d_vssp_header_t* header, const urg3d_range_header_t* range_header, const urg3d_range_index_t* range_index,const urg3d_data_range_intensity_t* data_range_intensity)
{
    long i;
    ...
    printf("sizeof(*data_range_intensity) = %ld\n", (unsigned long)sizeof(*data_range_intensity));
    for(i = 0; i < range_index->index[range_index->nspots]; i++) {
        printf("data_range_intensity->raw[%ld].range_mm = %u,  data_range_intensity->raw[%ld].intensity = %u\n",  i, data_range_intensity->raw[i].range_mm, i, data_range_intensity->raw[i].intensity);
    }
    printf("\n");
}
    ...
    while(1) {
        ...
        if(urg3d_next_receive_ready(&urg)) {
            ...
            if(urg3d_low_get_ri(&urg,  &header, &range_header, &range_index, &data_range_intensity) > 0) {
                print_debug_info(&urg,  &header, &range_header, &range_index, &data_range_intensity);
                n++;
